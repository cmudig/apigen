...
/**
 * Initialize an object with DataSymbol property
 * @param obj object that is going to be initialized
 * @param value if value is not provided, create an empty object, otherwise create an object with value
 */
export function init<T>(obj: BaseObject<T>, value?: any) {
    obj[DataSymbol] = value || {};
}

export function copy<T>(obj: BaseObject<T>) {
 ...
 */
export function set<T>(obj: BaseObject<T>, name: keyof T, value: any) {
    if (obj && obj[DataSymbol]){
        obj[DataSymbol][name] = object(value);
    }
}

export function get<T>(obj: BaseObject<T>, name: keyof T): any {
    if (obj && obj[DataSymbol]){
